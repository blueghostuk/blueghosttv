<?php
		...
		if (file_exists($dir.'data/'.$date.'/'))
		{
			echo '<p>Directory '.$dir.'data/'.$date.'/'.' already exists so skipping download</p>';
			flush();ob_flush();
		}
		else
		{
			...
			{//run bleb parser
				for($day=-1; $day <=7 ;$day++)
				{
					...
					if (file_exists($file))
					{
						...
						$parser->parseFile();
						echo '<br />Processed file '.$file.'<br />';
						flush();ob_flush();
					}
					else
					{
						echo '<br />Could not find file '.$file.'<br />';
						flush();ob_flush();
					}
				}
				echo '<br />Addding '.$parser->tv->title.' to DB<br />';
				flush();ob_flush();
