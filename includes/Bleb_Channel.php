	...
	class Bleb_Channel{
		...
		function getNowAndNext($time){
			...
			foreach($this->schedule as $s){
				if ($s->getDuration('hours') < 15){
					if ($id == $i){//next
						//echo 'found next in '.$i.'<br />';
						$scheds[] = $s;
					}else{
						if ($time <= $s->getEndTimeStamp() && $time >= $s->getStartTimeStamp()){//now
							//echo date("D dS M y H:i", $time).' <= '.date("D dS M y H:i", $s->getEndTimeStamp()).' && '.date("D dS M y H:i", $time).' >= '.date("D dS M y H:i", $s->getStartTimeStamp());
							//echo 'found new in '.$i.'<br />';
							$scheds[] = $s;
							$id = ($i+1);
						}
					}
